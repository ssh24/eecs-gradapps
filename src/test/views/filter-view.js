...
Filter.prototype.saveFilter = function(name){
	return this.utils.waitForElementClickable(this.filter.savePresetName, this.timeout).then(this.utils.clearThenSendKeys(element(this.filter.savePresetName), name))
		.then(this.utils.waitForElementClickable(this.filter.savePreset, this.timeout).then(element(this.filter.savePreset).click()));
};
