...
$(document).ready(function() {
    ...
    function assessmentDisplay(assmts) {
      var all_assmts = $('#all-assmts');

      all_assmts.empty();

      // create a list with all universities
      var all_unis = '<ol id="selected_list">';

      $.each(assmts, function(index, value) {
        all_unis += '<li><b>' + value['u_Name'] + '</b><ul>';
        var uni_assmts = value['u_Assessments'];
        if (uni_assmts.length > 0) {
          $.each(uni_assmts, function(index, value) {
            all_unis += '<li>' + uni_assmts[index] + '</li>';
          });
        } else {
          all_unis += '<li> No assessment selected </li>'
        }
        all_unis += '</ul> </li>';
      });
      all_unis += '</ol>'
      ...
      if (assmts.length > 0)
        all_assmts.append('<fieldset><legend id="selected_list_p" style="color:blue;font-weight:bold;">Selected University Assessments</legend>');
      all_assmts.append(all_unis);
    }
    ...
    $("#prev_uni").change(function() {
      ...
      /** create object to save key value pairs of assessments and display it **/
      var assmts = createUniAssessmentObject();
      assessmentDisplay(assmts);
    });
